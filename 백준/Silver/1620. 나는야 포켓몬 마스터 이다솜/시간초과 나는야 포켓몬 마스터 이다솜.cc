...
int main()
{
    ...
    std::map<int, std::string> pokemonMap;
    std::vector<std::string> answer;

    for (int i = 1; i <= libraryNum; i++) {
        std::cin >> name;
        pokemonMap.insert({i, name});
    }
    ...
    for (int i = 0; i < myNum; i++) {
        ...
        if (std::isdigit(yourInput[0])) {
            ...
            answer.push_back(pokemonMap[index]);
        }
        else {
            for (const auto& pair : pokemonMap) {
                if (pair.second == yourInput) {
                    answer.push_back(std::to_string(pair.first));
                    break;
                }
            }
        }
